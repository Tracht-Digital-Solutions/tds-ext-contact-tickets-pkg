...
/** The minor line the admin product caret-pins this package at (`^0.2.1`). */
...
describe("published surface", () => {
  ...
  it("publishes the islands the .astro shells hydrate", () => {
    ...
    for (const island of ["ContactInbox.tsx", "WidgetBody.tsx"]) {
...
describe("dependency hygiene", () => {
  ...
  it("stays inside the minor line the products caret-pin", () => {
    // tds-admin-frontend depends on this package with a CARET (`^0.2.1`). Under
    // 0.x a caret means `>=0.2.1 <0.3.0`, so bumping the MINOR here silently
